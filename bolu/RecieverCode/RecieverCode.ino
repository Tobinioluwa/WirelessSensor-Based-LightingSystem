...
const long interval = 180000; // Interval for three minutes (in milliseconds)
...
void setup() {
  ...
  Serial.println("Relay and CC1101 are ready");
}

byte buffer[61] = {0};

void loop() {
  // Check for incoming data from CC1101
  if (ELECHOUSE_cc1101.CheckRxFifo(100)) {
    ...
    if (ELECHOUSE_cc1101.CheckCRC()) {
      ...
      // Check if motion detected signal is received
      if (strstr(buffer, "Motion detected") != NULL) {
        motionDetected = true;
        count = 0;
        previousMillis = millis(); // Reset the timer if motion is detected
        // Turn on the relay
        digitalWrite(RELAY_PIN, HIGH);
        delay(100); // Keep the relay on for a short duration
              }
